...
from quo_plugins import with_plugins
...
@main.command()
@quo.option('--fields', help='List of properties to output.', default=None)
@quo.argument('input', metavar='<input file>')
@quo.argument('format', metavar='<output format>', type=quo.Choice(CONVERTERS.keys()))
def convert(fields, input, format):
    ...
    if fields:
        if not hasattr(converter_class, 'fields'):
            raise quo.QuoException('File format doesnt support custom list of fields')
    ...
    if format == 'images':
        quo.echo(quo.style('\rSuccessfully extracted images to directory: {}'.format(converter.dirname), fg='green'))
    else:
        quo.echo(quo.style('\rSuccessfully created new file: {}'.format(filename), fg='green'))


@main.command(name='domain')
@quo.argument('domain', metavar='<domain>')
@quo.option('--details', '-D', help='Lookup host information for any IPs in the domain results', default=False, is_flag=True)
@quo.option('--save', '-S', help='Save the information in the a file named after the domain (append if file exists).', default=False, is_flag=True)
@quo.option('--history', '-H', help='Include historical DNS data in the results', default=False, is_flag=True)
@quo.option('--type', '-T', help='Only returns DNS records of the provided type', default=None)
def domain_info(domain, details, save, history, type):
    ...
    quo.flair(info['domain'].upper(), fg='green')

    quo.echo('')
    for record in info['data']:
        quo.echo(
            u'{:32}  {:14}  {}'.format(
                quo.style(record['subdomain'], fg='cyan'),
                quo.style(record['type'], fg='yellow'),
                record['value']
            ),
            nl=False,
        )
        ...
        if record['value'] in hosts:
            ...
            quo.flair(u' Ports: {}'.format(', '.join([str(port) for port in sorted(host['ports'])])), fg='blue', nl=False)
        
        quo.echo('')


@main.command()
@quo.argument('key', metavar='<api key>')
def init(key):
    ...
    if not os.path.isdir(shodan_dir):
        try:
            ...
            raise quo.QuoException('Unable to create directory to store the Shodan API key ({})'.format(shodan_dir))
    ...
    with open(keyfile, 'w') as fout:
        ...
        quo.echo(quo.style('Successfully initialized', fg='green'))

    os.chmod(keyfile, 0o600)


@main.command()
@quo.argument('query', metavar='<search query>', nargs=-1)
def count(query):
    ...
    if query == '':
        raise quo.QuoException('Empty search query')
    ...
    quo.echo(results['total'])


@main.command()
@quo.option('--limit', help='The number of results you want to download. -1 to download all the data possible.', default=1000, type=int)
@quo.argument('filename', metavar='<filename>')
@quo.argument('query', metavar='<search query>', nargs=-1)
def download(limit, filename, query):
    ...
    if filename == '':
        raise quo.QuoException('Empty filename')
    ...
    try:
        ...
        raise quo.QuoException('The Shodan API is unresponsive at the moment, please try again later.')

    # Print some summary information about the download request
    quo.echo('Search query:\t\t\t%s' % query)
    quo.echo('Total number of results:\t%s' % total)
    quo.echo('Query credits left:\t\t%s' % info['unlocked_left'])
    quo.echo('Output file:\t\t\t%s' % filename)
    ...
    with helpers.open_file(filename, 'w') as fout:
        ...
        try:
            ...
            with quo.progressbar(cursor, length=limit) as bar:
                for banner in bar:
                    helpers.write_banner(fout, banner)
                    count += 1
        ...
        if count < limit:
            quo.echo(quo.style('Notice: fewer results were saved than requested', 'yellow'))
        quo.echo(quo.style(u'Saved {} results into file {}'.format(count, filename), 'green'))


@main.command()
@quo.option('--format', help='The output format for the host information. Possible values are: pretty, tsv.', default='pretty', type=quo.Choice(['pretty', 'tsv']))
@quo.option('--history', help='Show the complete history of the host.', default=False, is_flag=True)
@quo.option('--filename', '-O', help='Save the host information in the given file (append if file exists).', default=None)
@quo.option('--save', '-S', help='Save the host information in the a file named after the IP (append if file exists).', default=False, is_flag=True)
@quo.argument('ip', metavar='<ip address>')
...
def info():
    ...
    quo.echo("""Query credits available: {0}
Scan credits available: {1}
    """.format(results['query_credits'], results['scan_credits']))


@main.command()
@quo.option('--color/--no-color', default=True)
@quo.option('--fields', help='List of properties to output.', default='ip_str,port,hostnames,data')
@quo.option('--filters', '-f', help='Filter the results for specific values using key:value pairs.', multiple=True)
@quo.option('--filename', '-O', help='Save the filtered results in the given file (append if file exists).')
@quo.option('--separator', help='The separator between the properties of the search results.', default=u'\t')
@quo.argument('filenames', metavar='<filenames>', type=quo.Path(exists=True), nargs=-1)
def parse(color, fields, filters, filename, separator, filenames):
    ...
    if len(fields) == 0:
        raise quo.QuoException('Please define at least one property to show')
    ...
    if filename:
        ...
        if not has_filters:
            raise quo.QuoException('Output file specified without any filters. Need to use filters with this option.')
    ...
    for banner in helpers.iterate_files(filenames):
        ...
        for i, field in enumerate(fields):
            ...
            if value:
                ...
                if color:
                    tmp = quo.style(tmp, fg=COLORIZE_FIELDS.get(field, 'white'))
...
@main.command()
@quo.option('--ipv6', '-6', is_flag=True, default=False, help='Try to use IPv6 instead of IPv4')
def myip(ipv6):
...
@main.command()
@quo.option('--color/--no-color', default=True)
@quo.option('--fields', help='List of properties to show in the search results.', default='ip_str,port,hostnames,data')
@quo.option('--limit', help='The number of search results that should be returned. Maximum: 1000', default=100, type=int)
@quo.option('--separator', help='The separator between the properties of the search results.', default='\t')
@quo.argument('query', metavar='<search query>', nargs=-1)
def search(color, fields, limit, separator, query):
    ...
    if limit > 1000:
        raise quo.QuoException('Too many results requested, maximum is 1,000')
    ...
    if results['total'] == 0:
        raise quo.QuoException('No search results found')

    # We buffer the entire output so we can use quo's pager functionality
    output = u''
    for banner in results['matches']:
        ...
        for field in fields:
            ...
            # quo.echo(out + separator, nl=False)
        output += row + u'\n'
        # quo.echo('')
    quo.echo_via_pager(output)


@main.command()
@quo.option('--limit', help='The number of results to return.', default=10, type=int)
@quo.option('--facets', help='List of facets to get statistics for.', default='country,org')
@quo.option('--filename', '-O', help='Save the results in a CSV file of the provided name.', default=None)
@quo.argument('query', metavar='<search query>', nargs=-1)
def stats(limit, facets, filename, query):
    ...
    for facet in results['facets']:
        quo.echo('Top {} Results for Facet: {}'.format(len(results['facets'][facet]), facet))
        ...
        for item in results['facets'][facet]:
            ...
            quo.echo(quo.style(u'{:28s}'.format(value), fg='cyan'), nl=False)
            quo.echo(quo.style(u'{:12,d}'.format(item['count']), fg='green'))
...
@quo.option('--fields', help='List of properties to output.', default='ip_str,port,hostnames,data')
@quo.option('--separator', help='The separator between the properties of the search results.', default='\t')
@quo.option('--limit', help='The number of results you want to download. -1 to download all the data possible.', default=-1, type=int)
@quo.option('--datadir', help='Save the stream data into the specified directory as .json.gz files.', default=None, type=str)
@quo.option('--ports', help='A comma-separated list of ports to grab data on.', default=None, type=str)
@quo.option('--quiet', help='Disable the printing of information to the screen.', is_flag=True)
@quo.option('--timeout', help='Timeout. Should the shodan stream cease to send data, then timeout after <timeout> seconds.', default=0, type=int)
@quo.option('--streamer', help='Specify a custom Shodan stream server to use for grabbing data.', default='https://stream.shodan.io', type=str)
@quo.option('--countries', help='A comma-separated list of countries to grab data on.', default=None, type=str)
@quo.option('--asn', help='A comma-separated list of ASNs to grab data on.', default=None, type=str)
@quo.option('--alert', help='The network alert ID or "all" to subscribe to all network alerts on your account.', default=None, type=str)
@quo.option('--tags', help='A comma-separated list of tags to grab data on.', default=None, type=str)
@quo.option('--compresslevel', help='The gzip compression level (0-9; 0 = no compression, 9 = most compression', default=9, type=int)
@quo.option('--vulns', help='A comma-separated list of vulnerabilities to grab data on.', default=None, type=str)
def stream(color, fields, separator, limit, datadir, ports, quiet, timeout, streamer, countries, asn, alert, tags, compresslevel, vulns):
    ...
    if len(stream_type) > 1:
        raise quo.QuoException('Please use --ports, --countries, --tags, --vulns OR --asn. You cant subscribe to multiple filtered streams at once.')
    ...
    if ports:
        try:
            ...
            raise quo.QuoException('Invalid list of ports')
    ...
    while not quit:
        try:
            ...
            raise quo.QuoException('Connection timed out')
...
def honeyscore(ip):
    ...
    try:
        ...
        if score == 1.0:
            quo.echo(quo.style('Honeypot detected', fg='red'))
        elif score > 0.5:
            quo.echo(quo.style('Probably a honeypot', fg='yellow'))
        else:
            quo.echo(quo.style('Not a honeypot', fg='green'))

        quo.echo('Score: {}'.format(score))
    except Exception:
        raise quo.QuoException('Unable to calculate honeyscore')
...
def radar():
    ...
    try:
        ...
        raise quo.QuoException(u'{}'.format(e))
