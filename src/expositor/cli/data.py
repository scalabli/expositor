...
@quo.option('--dataset', help='See the available files in the given dataset', default=None, type=str)
def data_list(dataset):
    ...
    if dataset:
        ...
        for file in files:
            quo.echo(quo.style(u'{:20s}'.format(file['name']), fg='cyan'), nl=False)
            quo.echo(quo.style('{:10s}'.format(helpers.humanize_bytes(file['size'])), fg='yellow'), nl=False)

            # Show the SHA1 checksum if available
            if file.get('sha1'):
                quo.echo(quo.style('{:42s}'.format(file['sha1']), fg='green'), nl=False)
            
            quo.echo('{}'.format(file['url']))
    else:
        # If no dataset was provided then show a list of all datasets
        datasets = api.data.list_datasets()

        for ds in datasets:
            quo.echo(quo.style('{:15s}'.format(ds['name']), fg='cyan'), nl=False)
            quo.echo('{}'.format(ds['description']))


@data.command(name='download')
@quo.option('--chunksize', help='The size of the chunks that are downloaded into memory before writing them to disk.', default=1024, type=int)
@quo.option('--filename', '-O', help='Save the file as the provided filename instead of the default.')
@quo.argument('dataset', metavar='<dataset>')
@quo.argument('name', metavar='<file>')
def data_download(chunksize, filename, dataset, name):
    ...
    if not file:
        raise quo.QuoException('File not found')
    ...
    with open(filename, 'wb') as fout:
        with quo.progressbar(response.iter_content(chunk_size=chunk_size), length=limit) as bar:
            for chunk in bar:
                if chunk:
                    fout.write(chunk)

    quo.echo(quo.style('Download completed: {}'.format(filename), 'green'))
