...
def alert_clear():
    ...
    try:
        alerts = api.alerts()
        for alert in alerts:
            quo.echo(u'Removing {} ({})'.format(alert['name'], alert['id']))
            api.delete_alert(alert['id'])
    except shodan.APIError as e:
        raise quo.Outliers(e.value)
    quo.echo("Alerts deleted")


@alert.command(name='create')
@quo.argument('name', metavar='<name>')
@quo.argument('netblocks', metavar='<netblocks>', nargs=-1)
def alert_create(name, netblocks):
    ...
    quo.flair('Successfully created network alert!', fg='green')
    quo.flair('Alert ID: {}'.format(alert['id']), fg='cyan')


@alert.command(name='domain')
@quo.argument('domain', metavar='<domain>', type=str)
@quo.option('--triggers', help='List of triggers to enable', default='malware,industrial_control_system,internet_scanner,iot,open_database,new_service,ssl_expired,vulnerable')
def alert_domain(domain, triggers):
    ...
    try:
        ...
        quo.flair('Enabling triggers...', dim=True)
        api.enable_alert_trigger(alert['id'], triggers)
    except shodan.APIError as e:
        raise quo.Outliers(e.value)

    quo.flair('Successfully created domain alert!', fg='green')
    quo.flair('Alert ID: {}'.format(alert['id']), fg='cyan')


@alert.command(name='download')
@quo.argument('filename', metavar='<filename>', type=str)
@quo.option('--alert-id', help='Specific alert ID to download the data of', default=None)
def alert_download(filename, alert_id):
    ...
    try:
        ...
        quo.echo('Looking up alert information...')
        if alert_id:
            alerts = [api.alerts(aid=alert_id.strip())]
        else:
            alerts = api.alerts()
        
        quo.echo('Compiling list of networks/ IPs to download...')
        for alert in alerts:
    ...
    quo.flair('Successfully downloaded results into: {}'.format(filename), fg='green')


@alert.command(name='info')
@quo.argument('alert', metavar='<alert id>')
def alert_info(alert):
    ...
    quo.flair(info['name'], fg='cyan')
    quo.flair('Created: ', nl=False, dim=True)
    quo.flair(info['created'], fg='magenta')

    quo.flair('Notifications: ', nl=False, dim=True)
    if 'triggers' in info and info['triggers']:
        quo.flair('enabled', fg='green')
    else:
        quo.echo('disabled')

    quo.echo('')
    quo.flair('Network Range(s):', dim=True)

    for network in info['filters']['ip']:
        quo.echo(u' > {}'.format(quo.style(network, fg='yellow')))

    quo.echo('')
    if 'triggers' in info and info['triggers']:
        quo.flair('Triggers:', dim=True)
        for trigger in info['triggers']:
            quo.echo(u' > {}'.format(quo.style(trigger, fg='yellow')))
        quo.echo('')


@alert.command(name='list')
@quo.option('--expired', help='Whether or not to show expired alerts.', default=True, type=bool)
def alert_list(expired):
    ...
    if len(results) > 0:
        quo.echo(u'# {:14} {:<21} {:<15s}'.format('Alert ID', 'Name', 'IP/ Network'))

        for alert in results:
            quo.echo(
                u'{:16} {:<30} {:<35} '.format(
                    quo.style(alert['id'], fg='yellow'),
                    quo.style(alert['name'], fg='cyan'),
                    quo.style(', '.join(alert['filters']['ip']), fg='white')
                ),
                nl=False
            )

            if 'triggers' in alert and alert['triggers']:
                quo.flair('Triggers: ', fg='magenta', nl=False)
                quo.echo(', '.join(alert['triggers'].keys()), nl=False)

            if 'expired' in alert and alert['expired']:
                quo.flair('expired', fg='red')
            else:
                quo.echo('')
    else:
        quo.echo("You haven't created any alerts yet.")
...
@quo.argument('facets', metavar='<facets ...>', nargs=-1)
def alert_stats(limit, filename, facets):
    ...
    if not facets:
        raise quo.Outliers('No facets provided')
...
@quo.argument('alert_id', metavar='<alert ID>')
...
def alert_list_triggers():
    ...
    if len(results) > 0:
        quo.flair('The following triggers can be enabled on alerts:', dim=True)
        quo.echo('')

        for trigger in sorted(results, key=itemgetter('name')):
            quo.flair('{:<12} '.format('Name'), dim=True, nl=False)
            quo.flair(trigger['name'], fg='yellow')

            quo.flair('{:<12} '.format('Description'), dim=True, nl=False)
            quo.flair(trigger['description'], fg='cyan')

            quo.flair('{:<12} '.format('Rule'), dim=True, nl=False)
            quo.echo(trigger['rule'])

            quo.echo('')
    else:
        quo.echo("No triggers currently available.")


@alert.command(name='enable')
@quo.argument('alert_id', metavar='<alert ID>')
@quo.argument('trigger', metavar='<trigger name>')
def alert_enable_trigger(alert_id, trigger):
    ...
    quo.flair('Successfully enabled the trigger: {}'.format(trigger), fg='green')
...
def alert_disable_trigger(alert_id, trigger):
    ...
    quo.flair('Successfully disabled the trigger: {}'.format(trigger), fg='green')
